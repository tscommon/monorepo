...
describe("Mutex", () => {
  ...
  describe("lock", () => {
    ...
    test("with locking", async () => {
      const mutex = new Mutex(new Box(0));
      async function inc(wait: number) {
  ...
  test("attempts to lock", async () => {
    const mutex = new Mutex(new Box(0));
    {
      ...
      await expect(lock1).resolves.toStrictEqual(new Box(0));
      {
        await using lock2 = mutex.tryLock();
        await expect(lock2).resolves.toBe(Option.none);
      }
    }
  });
  ...
  test("releases automatically", async () => {
    ...
    {
      await using lock2 = mutex.tryLock();
      await expect(lock2).resolves.toStrictEqual(Option.some(new Box(0)));
    }
  });
